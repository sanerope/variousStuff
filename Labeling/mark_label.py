...
SIZE = 256, 256
OUTPUT_DIR = r'./Images/img'
INPUT_DIR = r'./Images/img'
...
class MarkTool:
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    ##                    print tmp
                    tmp = self.convert_from_darknet(tuple(tmp[1:]))
    ...
    def saveImage(self):
        with open(self.labelfilename, 'w') as f:
            for bbox in self.bboxList:
                f.write('0 ')
                converted = self.convert_to_darknet(bbox)
                f.write(' '.join(map(str, converted)) + '\n')
        print 'Image No. %d saved' %(self.cur)

    def convert_to_darknet(self, box): #box = (float(xmin), float(xmax), float(ymin), float(ymax))
        #bbox = (xmin, ymin, xmax, ymax)
        dw = 1. / self.tkimg.width()
        dh = 1. / self.tkimg.height()
        x = (box[0] + box[2]) / 2.0
        y = (box[1] + box[3]) / 2.0
        w = box[2] - box[0]
        h = box[3] - box[1]
    ...
    def convert_from_darknet(self, box): #box = (x, y, w, h)
        # bbox = (xmin, ymin, xmax, ymax)
        dw = 1. / self.tkimg.width()
        dh = 1. / self.tkimg.height()
        box = map(float, box)
